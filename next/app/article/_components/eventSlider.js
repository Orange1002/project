...
import { AiOutlineLeftCircle, AiOutlineRightCircle, AiOutlineHeart } from "react-icons/ai";
...
const EventSlider = () => {
  ...
  const showCards = () => (
    cards.slice(cardMove, cardMove + cardsPerPage).map((card, index) => (
      <div className="card card-h" key={index} style={{ position: "relative", width: "220px" }}>
        <img src={card.img} className="card-img-top object-fit-cover" alt="文章圖片" />
